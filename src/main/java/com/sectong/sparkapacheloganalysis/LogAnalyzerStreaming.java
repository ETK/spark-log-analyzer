...
public class LogAnalyzerStreaming {
	...
	public static void main(String[] args) {
		...
		JavaReceiverInputDStream<String> logDataDStream = jssc
				.socketTextStream(serverIP, serverPort);

		// A DStream of Apache Access Logs.
		JavaDStream<ApacheAccessLog> accessLogDStream = logDataDStream.map(
				Functions.PARSE_LOG_LINE).cache();
				...
				.foreachRDD(new Function<JavaRDD<ApacheAccessLog>, Void>() {
					...
					public Void call(JavaRDD<ApacheAccessLog> accessLogs) {
						...
						JavaRDD<Long> contentSizes = accessLogs.map(
								Functions.GET_CONTENT_SIZE).cache();
						...
						System.out.println("Response code counts: "
								+ responseCodeToCount);
						...
						System.out.println("IPAddresses > 10 times: "
								+ ipAddresses);
